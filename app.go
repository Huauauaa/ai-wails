...
func configFilePath() (string, error) {
	...
	return filepath.Join(d, "ai-wails", "config.json"), nil
}

// defaultBlogWorkDir returns ~/Documents/ai-wails-blog (OS path rules apply).
func defaultBlogWorkDir() (string, error) {
	home, err := os.UserHomeDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, "Documents", "ai-wails-blog"), nil
}
...
Use **Blog** in the activity bar. Markdown files live in your **blog working directory** (see Preference).

- **New File** creates a .md file on disk.
